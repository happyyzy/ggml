...
kernel void kernel_transpose_32_16(__read_only image1d_buffer_t input, __write_only image1d_buffer_t output, const uint rows, const uint cols, const uint padded_rows) {
    ...
    const int i_2 = i<<2;
    const int j_2 = j<<2;
    // Fast path for aligned rows: still read as FP32.
    // On Adreno, read_imageh from CL_FLOAT input can corrupt activations.
    if ((rows & 3u) == 0u) {
        float4 f0 = read_imagef(input, (j_2 + 0) * cols + i);
        float4 f1 = read_imagef(input, (j_2 + 1) * cols + i);
        float4 f2 = read_imagef(input, (j_2 + 2) * cols + i);
        float4 f3 = read_imagef(input, (j_2 + 3) * cols + i);

        write_imageh(output, (i_2 + 0) * padded_rows + j, convert_half4((float4)(f0.s0, f1.s0, f2.s0, f3.s0)));
        write_imageh(output, (i_2 + 1) * padded_rows + j, convert_half4((float4)(f0.s1, f1.s1, f2.s1, f3.s1)));
        write_imageh(output, (i_2 + 2) * padded_rows + j, convert_half4((float4)(f0.s2, f1.s2, f2.s2, f3.s2)));
        write_imageh(output, (i_2 + 3) * padded_rows + j, convert_half4((float4)(f0.s3, f1.s3, f2.s3, f3.s3)));
        return;
    }
...
kernel void kernel_transpose_32_32(__read_only image1d_buffer_t input,
                                   __write_only image1d_buffer_t output,
                                   const uint rows,
                                   const uint cols,
