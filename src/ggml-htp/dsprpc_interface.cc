...
void __attribute__((weak)) rpcmem_init(void) {
...
void __attribute__((weak)) rpcmem_deinit(void) {
...
void * __attribute__((weak)) rpcmem_alloc(int heap_id, uint32_t flags, int size) {
    auto fn = dsprpc_interface::instance()->rpcmem_alloc_fn;
    return fn ? fn(heap_id, flags, size) : nullptr;
}

void __attribute__((weak)) rpcmem_free(void * p) {
...
int __attribute__((weak)) rpcmem_to_fd(void * p) {
    auto fn = dsprpc_interface::instance()->rpcmem_to_fd_fn;
    return fn ? fn(p) : -1;
}

int __attribute__((weak)) fastrpc_mmap(int domain, int fd, void * addr, int offset, size_t length, enum fastrpc_map_flags flags) {
    auto fn = dsprpc_interface::instance()->fastrpc_mmap_fn;
    return fn ? fn(domain, fd, addr, offset, length, flags) : -1;
}

int __attribute__((weak)) fastrpc_munmap(int domain, int fd, void * addr, size_t length) {
